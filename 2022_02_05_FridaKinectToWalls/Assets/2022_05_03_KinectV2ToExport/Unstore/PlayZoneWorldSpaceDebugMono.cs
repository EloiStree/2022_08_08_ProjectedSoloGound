...
public class PlayZoneWorldSpaceDebugMono : MonoBehaviour
{
    ...
    public void WorldToLocal(in Transform point,out Vector3 worldLocal, out Vector3 local)
    {
        WorldToLocal( point.position, out worldLocal, out local);
    }
    public void WorldToLocal(in Vector3 point, out Vector3 worldLocal, out Vector3 local)
    {
        if (!m_source.IsPointDefined()) {
            worldLocal = Vector3.zero;
            local = Vector3.zero;
            return;
        }
        ...
        if (m_removeHeight)
            local.y = 0;
        GetWorldPositionFromLocal(in local, out worldLocal);
    }
    ...
    public void Update()
    {
        if (!m_source.IsPointDefined())
            return;
        RefreshLeftDownInfo();
        WorldToLocal(in m_source.m_topLeft, out m_tlLocalWorld, out m_tlLocal);
        WorldToLocal(in m_source.m_topRight, out m_trLocalWorld, out m_trLocal);
        WorldToLocal(in m_source.m_downRight, out m_drLocalWorld, out m_drLocal);
        WorldToLocal(in m_source.m_downLeft, out m_dlLocalWorld, out m_dlLocal);
    ...
    public void GetWorldPositionFromLocal(in Vector3 local, out Vector3 world) {
