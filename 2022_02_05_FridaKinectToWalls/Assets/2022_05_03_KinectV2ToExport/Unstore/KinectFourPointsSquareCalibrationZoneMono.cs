...
public class KinectFourPointsSquareCalibrationZoneMono : MonoBehaviour
{
    ...
    {
        if (m_topLeft.localPosition == Vector3.zero)
            return false;
        if (m_topRight.localPosition == Vector3.zero)
            return false;
        if (m_downLeft.localPosition == Vector3.zero)
            return false;
        if (m_downRight.localPosition == Vector3.zero)
            return false;
        return true;
    }
