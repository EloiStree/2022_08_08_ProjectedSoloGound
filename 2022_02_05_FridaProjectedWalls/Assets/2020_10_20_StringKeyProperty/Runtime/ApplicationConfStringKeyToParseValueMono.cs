...
public class ApplicationConfStringKeyToParseValueMono : MonoBehaviour
{
    ...
    public UnityEvent m_beforeImport;
    public ListenToStrigKeyValueType m_listeners;
    public UnityEvent m_afterImport;

    [TextArea(0,5)]
    public string m_exportText;

    public void Reset() {
        Eloi.E_SearchInSceneUtility.TryToFetchWithInScene( ref m_source); 
    }

    public string PutCommentaryInFrontOfLine(string text) {
    ...
    public void OnValidate()
    {
        ...
        {
            item.GetNotFoundDefaultValue(out bool value);
            item.GetKeyId(out string id);
            AddDescriptionBeforeIfHave(ref m_exportText, item );
            m_exportText += id + ":" + value + "\n";
        }
        foreach (var item in m_listeners.m_doubleEvent)
        {
            item.GetNotFoundDefaultValue(out double value);
            item.GetKeyId(out string id);
            AddDescriptionBeforeIfHave(ref m_exportText, item);
            m_exportText += id + ":" + value + "\n";
        }
        ...
        {
            item.GetNotFoundDefaultValue(out float value);
            item.GetKeyId(out string id);
            AddDescriptionBeforeIfHave(ref m_exportText, item);
            m_exportText += id + ":" + value + "\n";
        }
        ...
        {
            item.GetNotFoundDefaultValue(out int value);
            item.GetKeyId(out string id);
            AddDescriptionBeforeIfHave(ref m_exportText, item);
            m_exportText += id + ":" + value + "\n";
        }
        ...
        {
            item.GetNotFoundDefaultValue(out string value);
            item.GetKeyId(out string id);
            AddDescriptionBeforeIfHave(ref m_exportText, item);
            m_exportText += id + ":" + value + "\n";
        }
        ...
        {
            item.GetNotFoundDefaultValue(out ulong value);
            item.GetKeyId(out string id);
            AddDescriptionBeforeIfHave(ref m_exportText, item);
            m_exportText += id + ":" + value + "\n";
        }
    ...
    private void AddDescriptionBeforeIfHave<T>(ref string toAddTo, StringKeyToValueEvent<T> item)
    {
        if (item.m_shortDescription.Length > 0)
            toAddTo += PutCommentaryInFrontOfLine(item.m_shortDescription);
    }

    public void AccessAndPushObserveValue()
    {
        m_beforeImport.Invoke();
